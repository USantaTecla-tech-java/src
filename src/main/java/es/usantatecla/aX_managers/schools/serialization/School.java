package es.usantatecla.aX_managers.schools.serialization;
...
class School {
	...
	private void write() {
		...
		try {
			out = new ObjectOutputStream(new FileOutputStream("school.dat"));
	...
	private void read() {
		...
		try {
			in = new ObjectInputStream(new FileInputStream("school.dat"));
			int total = in.readInt();
			for (int i = 0; i < total; i++) {
				this.persons[this.numPersons] = (Person) in.readObject();
				this.numPersons++;
			}
		} catch (IOException ex) {
